...
export let ruleArray: { [key: string]: RuleFunction } = {
    min: (value: string, parameter) => parseInt(value, null) >= parameter,
    max: (value: string, parameter) => parseInt(value, null) <= parameter,
...
export function addRule(ruleObj: RuleObj) {
    ...
    if (ruleObj.regExp) {
        regExpArray[ruleObj.key] = ruleObj.rule;
    }
    if (ruleObj.rule) {
        ruleArray[ruleObj.key] = ruleObj.rule;
    }
    if (ruleObj.replace) {
        replaceArray[ruleObj.key] = ruleObj.replace
    }
}
